...
export const DELETE_BLOCK_MODAL = "DELETE_BLOCK_MODAL";

export const DeleteBlockModal: FC<DeleteModalProps> = observer(({ id }) => {
    ...
    const { mutate, isLoading } = useMutation({
        fetchFn: CertificatesAPI.removeBlock,
        onSuccess: data => {
            closeModal();

            throwSuccessToast(
                t("hasDeleted", {
                    word: t("certificate.blocks.plural_0")
                })
            );
            const cb = modalCallback[DELETE_BLOCK_MODAL];
            cb && cb(true, data);
        },
    ...
    return (
        <section className="w-600p bg-white rounded-20p flex flex-col text-dark-main">
            <h4 className="text-18 font-semibold p-6">
                {t("certificate.sureToDeleteBlock")}
            </h4>
            <span className="text-14 leading-5 mb-16 px-6">
                {t("certificate.sureToDeleteBlockDescr")}
            </span>
