...
export const DELETE_LESSON_MODAL = "DELETE_LESSON_MODAL";

export const DeleteLessonModal: FC<DeleteModalProps> = observer(({ id }) => {
    ...
    const { mutate, isLoading } = useMutation({
        fetchFn: CertificatesAPI.removeLesson,
        onSuccess: data => {
            closeModal();

            throwSuccessToast(
                t("hasDeleted", {
                    word: t("certificate.lesson.plural_0")
                })
            );
            const cb = modalCallback[DELETE_LESSON_MODAL];
            cb && cb(true, data);
        },
    ...
    return (
        <section className="w-600p bg-white rounded-20p flex flex-col text-dark-main">
            <h4 className="text-18 font-semibold p-6">
                {t("certificate.sureToDeleteLesson")}
            </h4>
            <span className="text-14 leading-5 mb-16 px-6">
                {t("certificate.sureToDeleteLessonDescr")}
            </span>
