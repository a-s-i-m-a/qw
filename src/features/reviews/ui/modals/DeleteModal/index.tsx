...
export const REVIEW_DELETE_MODAL = "REVIEW_DELETE_MODAL";

export const ReviewDeleteModal: FC<ReviewDeleteModalProps> = observer(
    ({ id }) => {
        const {
            register,
            activeModalId,
        ...
        const { mutate, isLoading } = useMutation({
            fetchFn: PromosAPI.removeReview,
            onSuccess: () => {
                throwSuccessToast(t("reviewHasDeleted"));

                const cb = modalCallback[REVIEW_DELETE_MODAL];
                cb && cb(true);
            },
